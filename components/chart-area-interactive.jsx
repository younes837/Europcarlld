...
export function ChartAreaInteractive() {
    ...
    const fetchData = async () => {
      try {
        ...
        const dynamicChartData = lab.map((label, index) => ({
          ...
          restitutions:
            data2.find((d) => `T${d.Trimestre} ${d.Annee}` === label)
              ?.NombreContrats ?? 0,
        }));
  ...
  return (
    <Card className="@container/card">
      <CardHeader>
        <CardTitle>
          Nombre de productions et Restitutions par Trimestre
        </CardTitle>
        <CardDescription>
          ...
          <ResponsiveContainer>
            ...
            >
              ...
              <XAxis
                dataKey="name"
                height={60}
                tick={<CustomizedAxisTick className="cursor-pointer" />}
