...
export default function LldVrPage() {
  ...
  const [matriculeValue, setMatriculeValue] = useState(""); // Immediate input value
  const [searchTerm, setSearchTerm] = useState(""); // Debounced search term
  const [matriculeTerm, setMatriculeTerm] = useState(""); // Debounced matricule term
  const [exportLoading, setExportLoading] = useState(false);

  // Date filters with debounce - initialize both states with the same values
  const [startDate, setStartDate] = useState("");
  const [endDate, setEndDate] = useState("");
  const [debouncedStartDate, setDebouncedStartDate] = useState(""); // Initialize with same value
  ...
  // Handle matricule input change
  const handleMatriculeChange = (event) => {
    setMatriculeValue(event.target.value);
  };
  ...
  // Debounce matricule term
  useEffect(() => {
    const timerId = setTimeout(() => {
      if (matriculeValue !== matriculeTerm) {
        setMatriculeTerm(matriculeValue);
        // Reset to first page when matricule search changes
        setPaginationModel({
          ...paginationModel,
          page: 0,
        });
    ...
    return () => clearTimeout(timerId);
  }, [matriculeValue, matriculeTerm, paginationModel]);
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      const fetchExportData = async () => {
        ...
        const apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/lld_vr?page=0&pageSize=10000&sortField=${sortField}&sortDirection=${sortDirection}&searchQuery=${searchTerm}&matricule=${matriculeTerm}&fromDate=${debouncedStartDate}&toDate=${debouncedEndDate}`;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/lld_vr?page=${paginationModel.page}&pageSize=${paginationModel.pageSize}&sortField=${sortField}&sortDirection=${sortDirection}&searchQuery=${searchTerm}&matricule=${matriculeTerm}&fromDate=${debouncedStartDate}&toDate=${debouncedEndDate}`;
